...
from backend.ml.hashtag_gen import GeminiHashtagGenerator
...
hashtag_generator = GeminiHashtagGenerator(api_key)
...
async def generate_hashtags_from_image(
    image: UploadFile = File(...),
    theme: str = Form(...),
    tone: str = Form(...)
):
    try:
        ...
        # Predict vibe and object
        vibe, _ = vibe_classifier.classify(pil_image, mode="vibe")
        pil_image.seek(0)
        obj, _ = vibe_classifier.classify(pil_image, mode="object")

        # Generate hashtags
        hashtags = hashtag_generator.generate_hashtags(vibe, obj, theme, tone)
        ...
        return {"error": f"Error generating hashtags: {str(e)}"}
